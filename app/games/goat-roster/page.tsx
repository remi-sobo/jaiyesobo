...
export const metadata: Metadata = {
  title: "GOAT Roster — Jaiye's Games",
  description: "Pick 5 starters from a franchise's all-time pool. AI scores your lineup.",
};

export const dynamic = "force-dynamic";

export default async function GoatRosterLandingPage() {
  const summaries = await getDraftTeamSummaries();
  const playable = summaries.filter((s) => s.verified >= 5);

  return (
    <GameShell liveLabel="Single-player · AI judges">
      <section className="px-6 lg:px-10 pt-20 pb-10 max-w-[1100px] mx-auto">
        <div className="font-[family-name:var(--font-jetbrains)] text-[0.65rem] uppercase tracking-[0.3em] text-[var(--color-mute)] mb-6">
          jaiyesobo.com / games / goat-roster
        </div>
        <h1 className="font-[family-name:var(--font-fraunces)] font-black text-[clamp(2.5rem,7vw,5.5rem)] leading-[0.95] tracking-[-0.03em] mb-6">
          GOAT <span className="italic font-normal text-[var(--color-red)]">Roster.</span>
        </h1>
        <p className="font-[family-name:var(--font-fraunces)] italic text-[clamp(1.05rem,1.6vw,1.4rem)] text-[var(--color-bone)] max-w-[52ch] leading-snug mb-3">
          Pick 5 starters and a sixth man from a franchise&apos;s all-time pool. Build your dream lineup.
        </p>
        <p className="text-[var(--color-mute)] text-[0.95rem] max-w-[52ch] leading-relaxed mb-12">
          One guard, one forward, one center is the classic shape — but go off
          the rails if you want. The 6th man is your sparkplug off the bench. AI
          scores your lineup, names your best pick, and tells you what&apos;s a stretch.
        </p>
      </section>
      ...
      <section className="px-6 lg:px-10 pb-24 max-w-[1100px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {playable.map((s) => (
            <TeamCard key={s.slug} slug={s.slug} team={s.team} verified={s.verified} />
          ))}
          {playable.length === 0 && (
            <div className="col-span-full p-10 border border-dashed border-[var(--color-line)] rounded text-center text-[var(--color-mute)]">
              No franchises ready yet — pools must have at least 5 verified players.
            </div>
          )}
        </div>
      </section>
